...
第五步：把后端的SessionID设置在Cookie中，同时告知客户端ticket有效。当用户再次访问该客户端，就可以根据Cookie 中的SessionID找到客户端的Session，获取用户信息，就不用再次进行验证了。也就是图中响应给浏览器的部分。用户通过认证，前端与后端之间进行正常的访问。
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5MjE2NzY4NDQsNjQxMzI5MzU4LC0xOT
