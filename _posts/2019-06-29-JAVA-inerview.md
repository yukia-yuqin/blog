...
- 简历中出现的死锁是什么情况？
    - 背景： 在
    - 第一步会有两个进程分别去获取数据的锁和copy数据，获取数据的进程是在一个切片中获取，第二步有两个进程去获取copy之后的数据的锁和执行mapreduce的操作，在这个过程中第二步会出现数据被占用的情况，造成第二步的死锁。
    - 解决方法： 升级到数据工厂二，每一步的操作和数据绑定，不对数据进行单独的加锁处理
    - 个人能力：发现问题，快速学习，解决问题
...
- [多线程还是多进程？](https://www.zhihu.com/question/43591043)
